...
#include <iostream>
#include <fstream>
#include "SimulatedBifurcationToFixed.h"
...
void simulated_bifurcation_run(
	...
	for(unsigned int it = 0; it < num_iterations; it++){
		...
		for(unsigned int m = 0; m < M; m++){
			...
			for(unsigned int i = 0; i < N; i++){
				y_M[i] = y_M[i] - (((long long int)((((K*((x_M[i]*((x_M[i]*x_M[i])>> fractional_bits)) >> fractional_bits))>>fractional_bits)+(((Delta - pt)*x_M[i])>>fractional_bits)))*delta_t_M)>> fractional_bits);
			}			
		...
		for(unsigned int i = 0; i < N; i++){
			temp = 0;
			for (unsigned int j = 0; j < N; j++){
				temp += ((J_xi[i][j])*x[j]) >> fractional_bits;
			}
