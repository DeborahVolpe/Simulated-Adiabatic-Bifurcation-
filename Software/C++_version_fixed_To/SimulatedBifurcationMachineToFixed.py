...
import neal
from GeneralSimulatedBifurcationMachineToF import SimulatedBifurcationMachineTo_fix 
...
class SimulatedBifurcationMachineVLSISolverFixedTo:
    # solver constructor
    ...
    def SampleQUBO(self, qubo, numIterations=100, numRuns = 100, frac_bits=32, **kwargs):
        ...
        if J_dev != 0:
            self.xi = int((0.7/(math.sqrt(self.N) * J_dev))*2**self.frac_bits)
        else:
            self.xi = int((0.7/(math.sqrt(self.N) * 0.01))*2**self.frac_bits)            
        
        #default value for the xi parameter
        if h_dev != 0:
            self.xi_h = int((0.7/(math.sqrt(self.N) * h_dev))*2**self.frac_bits)
        else:
            self.xi_h = int((0.7/(math.sqrt(self.N) * 0.01))*2**self.frac_bits)
        ...
        self.J_int = (self.J*2**self.frac_bits).astype(int)
        self.h_int = (self.h*2**self.frac_bits).astype(int)        
        self.energies = np.zeros(self.numRuns)
        self.solutions = np.zeros((self.numRuns, self.N))  
        seed = randint(2**31)     
        start = time.time()
        self.solutions, self.energies  = SimulatedBifurcationMachineTo_fix(self.folder,self.solutions, self.energies, self.numRuns, self.h_int, self.J_int, self.N, self.K, self.Delta, self.deltaT, self.xi, self.xi_h, self.p_shape, self.numIterations, seed, self.verbose, self.frac_bits, self.M)           
        stop = time.time()      
    ...
    def Sample(self, J, h, N, offset, numIterations=100, numRuns = 100, frac_bits=32, **kwargs):
        ...
        self.solutions, self.energies  = SimulatedBifurcationMachineTo_fix(self.folder,self.solutions, self.energies, self.numRuns, self.h_int, self.J_int, self.N, self.K, self.Delta, self.deltaT, self.xi, self.xi_h, self.p_shape, self.numIterations, seed, self.verbose, self.frac_bits, self.M)           
        stop = time.time()      
        self.energies = list(np.array(self.energies)/2**self.frac_bits + self.offset)
        self.TotalExecutionTime = stop - start       
        self.optimalEnergy = min(self.energies)
        self.optimalSolution = self.solutions[list( self.energies).index(self.optimalEnergy)]
        return self.optimalSolution, self.optimalEnergy
    ...
    def read_parameters_from_files(self): 
        if self.verbose == True:
            ...
            for line in file_x.readlines():
                ...
                if len(p) == self.N: 
                    for i in range(len(p)):
                        self.x[i].append(float(p[i])/2**self.frac_bits)
                else:
                    return None
            file_x.close()
            ...
            for line in file_y.readlines():
                ...
                if len(p) == self.N: 
                    for i in range(len(p)):
                        self.y[i].append(float(p[i])/2**self.frac_bits)
                else:
                    return None
            file_y.close()
            ...
            for line in file_value.readlines():
                self.valueAtIteration.append(float(line[:-1])/2**self.frac_bits)
            file_value.close()
            ...
            for line in file_effective_energy.readlines():
                self.averageEffectiveValue.append(float(line[:-1])/2**self.frac_bits + self.offset)
    ...
    def WriteResultsInAFile(self, nomeFileReport,  **kwargs):
        ...
        ReportFile.write("The considered M " + format(self.M) + "\n")
