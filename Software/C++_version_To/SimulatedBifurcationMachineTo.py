...
import neal
from GeneralSimulatedBifurcationMachine_To_v1 import SimulatedBifurcationMachineTo_v1 
...
class ToSimulatedBifurcationMachineToSolver:
    # solver constructor
    ...
    def SampleQUBO(self, qubo, numIterations=100, numRuns = 100, **kwargs):
        ...
        seed = randint(2**31)     
        start = time.time()
        self.solutions, self.energies  = SimulatedBifurcationMachineTo_v1(self.folder,self.solutions, self.energies, self.numRuns, self.h, self.J, self.N, self.K, self.Delta, self.deltaT, self.xi, self.xi_h, self.p_shape, self.numIterations, seed, self.verbose, self.M)           
        stop = time.time()      
