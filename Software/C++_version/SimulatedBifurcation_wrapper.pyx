...
cdef extern from "./SimulatedBifurcation.h":
	int general_simulated_bifurcation(string folder, vector[vector[int]]& states, vector[double]& energies, const unsigned int num_runs, const vector[double]& h, const vector[vector[double]]& J, const unsigned int N, const double K, const double Delta, const double delta_t, const double xi, const double xi_h, const double pt_shape, const unsigned int num_iterations, const unsigned long long seed, bool verbose )

def SimulatedBifurcationMachineVLSI(folder, states_numpy, energies, num_runs, h, J, N, K, Delta, delta_t, xi, xi_h, pt_shape, num_iterations, seed, verbose):
	num_vars = len(h)
	...
	ret = general_simulated_bifurcation(_folder, _states, _energies, _num_runs, _h, _J, _N, _K, _Delta, _delta_t, _xi, _xi_h, _pt_shape, _num_iterations, _seed, verbose)
