...
#include <iostream>
#include <fstream>
#include "SimulatedBifurcationFixed.h"
...
void simulated_bifurcation_run(
	...
	long long int K_1 = (long long int)((1.0/((double)K/pow(2.0, (double) fractional_bits)))*pow(2.0, (double) fractional_bits));
	...
	for(unsigned int i = 0; i < N; i++){
		if(single_run == false){
			...
			//FASTRAND(rand);
			r = rand();
			y[i] =(long long int) (((double(int(r)% 20) - 10.0)/100.0)*pow(2.0, (double) fractional_bits));
			if(y[i] == 0) {
				r = rand();
				y[i] =(long long int) (((double(int(r)% 10) + 1.0)/100.0)*pow(2.0, (double) fractional_bits));
			}
		}else{
...
int general_simulated_bifurcation_fixed(
	//string folder
	string folder,
	//final states
