...
deltaT = 0.6#The algorithm is stable when deltaT < 0.5
...
for j in range(numTest):
    ...
    xVectorO = np.matrix([0,0])
    pt = 0
    yVector = np.matrix([0,random.choice([-0.1,0.1])])
    ...
    for i in range(numberSteps):
        ...
        tempVector = np.add(np.add(K*np.power(xVector, 3), (Delta-pt)*xVector), xi*xOld*JMatrix)
...
plt.plot(time, x0Val, linewidth=2.0, color='b', label=r'\textit{x0 approximated}')
plt.plot(time, y0Val, linewidth=2.0, color='r', label=r'\textit{y0 approximated}')
plt.plot(time, x1Val, linewidth=2.0, color='c', label=r'\textit{x1 approximated}')
plt.plot(time, y1Val, linewidth=2.0, color='m', label=r'\textit{y1 approximated}')
...
plt.savefig('x0_x1_y0_y1_comparison.eps', format='eps')
plt.savefig('x0_x1_y0_y1_comparison.pdf', format='pdf')
plt.savefig('x0_x1_y0_y1_comparison.png', format='png')
plt.show()
