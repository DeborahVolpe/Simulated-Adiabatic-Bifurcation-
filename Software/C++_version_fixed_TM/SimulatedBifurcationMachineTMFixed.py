...
import neal
from GeneralSimulatedBifurcationMachineTMF import SimulatedBifurcationMachineTM_fix 
...
class SimulatedBifurcationMachineVLSISolverFixedTM:
    # solver constructor
    ...
    def SampleQUBO(self, qubo, numIterations=100, numRuns = 100, frac_bits=32, **kwargs):
        ...
        seed = randint(2**31)     
        start = time.time()
        self.solutions, self.energies  = SimulatedBifurcationMachineTM_fix(self.folder,self.solutions, self.energies, self.numRuns, self.h_int, self.J_int, self.N, self.K, self.Delta, self.deltaT, self.xi, self.xi_h, self.p_shape, self.numIterations, seed, self.verbose, self.frac_bits, self.M)           
        stop = time.time()      
