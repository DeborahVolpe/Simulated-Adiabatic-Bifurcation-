...
#include <iostream>
#include <fstream>
#include "SimulatedBifurcationTMFixed.h"
...
void simulated_bifurcation_run(
	...
	vector<long long int> x_M(N);
	vector<long long int> y_M(N);
	...
	for(unsigned int it = 0; it < num_iterations; it++){
		...
		for(unsigned int m = 0; m < M; m++){
			...
			for(unsigned int i = 0; i < N; i++){
				x_M[i] = x_M[i]+ ((((y_M[i]*delta_t_M) >> fractional_bits)*Delta) >> fractional_bits);
			}
			
			// update momentum variables
			for(unsigned int i = 0; i < N; i++){
				y_M[i] = y_M[i] - ((long long int)((long long int)((((K*((x_M[i]*((x_M[i]*x_M[i])>> fractional_bits)) >> fractional_bits))>>fractional_bits)+(((Delta - pt)*x_M[i])>>fractional_bits)))*delta_t_M)>> fractional_bits);
			}			
		...
		for(unsigned int i = 0; i < N; i++){
			...
			y[i] = y_M[i] - ((long long int)((temp +  ((at*(((xi_h << 2)*h[i]) >> fractional_bits))>>fractional_bits))*delta_t)>> fractional_bits);
			if (verbose == true){
				if(i != N-1){
					y_file << y[i] << " ";
				}else{
...
int general_simulated_bifurcation_TMfixed(
	//string folder
	string folder,
	//final states
	...
	const long long int delta_t_M =  (long long int)delta_t/M;

	for(unsigned int time = 0; time < num_runs;  time++){
		
		if (num_runs > 1){
			if(verbose == true && time == 0){
				simulated_bifurcation_run(folder, num_runs, h, J, N, K, Delta, delta_t, xi, xi_h, pt_shape, num_iterations, true, states[time], &energy, average_effective_energy, verbose, false, fractional_bits, M, delta_t_M);
			}else{
				simulated_bifurcation_run(folder, num_runs, h, J, N, K, Delta, delta_t, xi, xi_h, pt_shape, num_iterations, false, states[time], &energy, average_effective_energy, verbose, false, fractional_bits, M, delta_t_M);
			}
		}else{
			simulated_bifurcation_run(folder, num_runs, h, J, N, K, Delta, delta_t, xi, xi_h, pt_shape, num_iterations, verbose, states[time], &energy, average_effective_energy, verbose, true, fractional_bits, M, delta_t_M);			
		}
