...
nome_file_intermidiate = "IntermediateResults6.txt"
...
# Adiacent Matrix
JMatrixf = np.matrix([[0, 1.75, 1, 0, 0, 0], [1.75, 0, 0.75, 1.5, 0, 0], [1, 0.75, 0, 2, 3, 0], [0, 1.5, 2, 0, 0.75, 1.5],[0, 0, 3, 0.75, 0, 0.5], [0, 0, 0, 1.5, 0.5, 0]])
#JMatrixf = np.matrix([[0, 0.75, 0, 0.75, 0.25], [0.75, 0, 0.5, 0, 0], [0, 0.5, 0, 0.5, 0], [0.75, 0, 0.5, 0, 0.75], [0.25, 0, 0, 0.75, 0]])
JMatrix = np.matrix([[0, int(1.75*2**NFrac), int(1*2**NFrac), 0, 0, 0], [int(1.75*2**NFrac), 0, int(0.75*2**NFrac), int(1.5*2**NFrac), 0, 0], [int(1*2**NFrac), int(0.75*2**NFrac), 0, int(2*2**NFrac), int(3*2**NFrac), 0], [0, int(1.5*2**NFrac), int(2*2**NFrac), 0, int(0.75*2**NFrac), int(1.5*2**NFrac)],[0, 0, int(3*2**NFrac), int(0.75*2**NFrac), 0, int(0.5*2**NFrac)], [0, 0, 0, int(1.5*2**NFrac), int(0.5*2**NFrac), 0]], dtype=np.int32)

# H vector
HVectorf = np.matrix([0, 0, 0, 0, 0, 0])
HVector = np.matrix([0, 0, 0, 0, 0, 0])
...
#xif = Kf * 0.5
xif = Delta * 0.07 / (math.sqrt(numberSpin) * JMatrix.std())
xi = int(xif*2**NFrac)
ShapePt = int(0.028*2**NFrac)
...
xVector = np.matrix([[0,0, 0, 0, 0, 0]],dtype=np.int32)
xVector2 = np.matrix([[0,0, 0, 0, 0, 0]],dtype=np.int32)
yVector = np.matrix([[0, 0, 0, 0, 0, np.int32(random.choice([ 0.1])*2**NFrac)]],dtype=np.int32)
yVector2 = np.matrix([[0, 0, 0, 0, 0,  np.int32(random.choice([+0.1])*2**NFrac)]],dtype=np.int32)

xVectorf = np.matrix([0,0,0, 0, 0, 0])
ptf = 0
yVectorf = np.matrix([0,0, 0, 0, 0, random.choice([-0.1, 0.1])])

y = int(yVector.item(5))
if y < 0:
	y = 2**NBit + y
...
J61 = int(JMatrix.item(5, 0)*xi >> NFrac)
if J61 < 0:
    J61 = 2 ** NBit + J61
...
J62 = int(JMatrix.item(5, 1)*xi >> NFrac)
if J62 < 0:
    J62 = 2 ** NBit + J62
...
J63 = int(JMatrix.item(5, 2)*xi >> NFrac)
if J63 < 0:
    J63 = 2 ** NBit + J63
...
J64 = int(JMatrix.item(5, 3) * xi >> NFrac)
if J64 < 0:
    J64 = 2 ** NBit + J64
...
J65 = int(JMatrix.item(5, 4) * xi >> NFrac)
if J65 < 0:
    J65 = 2 ** NBit + J65

J16 = int(JMatrix.item(0, 5) * xi >> NFrac)
if J16 < 0:
    J16 = 2 ** NBit + J16

J26 = int(JMatrix.item(1, 5) * xi >> NFrac)
if J26 < 0:
    J26 = 2 ** NBit + J26

J36 = int(JMatrix.item(2, 5) * xi >> NFrac)
if J36 < 0:
    J36 = 2 ** NBit + J36

J46 = int(JMatrix.item(3, 5) * xi >> NFrac)
if J46< 0:
    J46 = 2 ** NBit + J46

J56 = int(JMatrix.item(4, 5) * xi >> NFrac)
if J56 < 0:
    J56 = 2 ** NBit + J56
...
ParameterFile.write(format(J16, 'b').zfill(NBit) + "\n") #J21
print("J16=" + format(J16))
...
ParameterFile.write(format(J26, 'b').zfill(NBit) + "\n") #J21
print("J26=" + format(J26))
...
ParameterFile.write(format(J36, 'b').zfill(NBit) + "\n") #J21
print("J36=" + format(J36))
...
ParameterFile.write(format(J46, 'b').zfill(NBit) + "\n") #J21
print("J46=" + format(J46))
...
ParameterFile.write(format(J56, 'b').zfill(NBit) + "\n") #J21
print("J56=" + format(J56))
ParameterFile.write(format(J61, 'b').zfill(NBit) + "\n") #J12
print("J61= " + format(J61) + "\n")
ParameterFile.write(format(J62, 'b').zfill(NBit) + "\n") #J21
print("J62=" + format(J62))
ParameterFile.write(format(J63, 'b').zfill(NBit) + "\n") #J21
print("J63=" + format(J63))
ParameterFile.write(format(J64, 'b').zfill(NBit) + "\n") #J21
print("J64=" + format(J64))
ParameterFile.write(format(J65, 'b').zfill(NBit) + "\n") #J21
print("J65=" + format(J65))
ParameterFile.close()
...
for i in range(numberSteps):
    ...
    p.append(pt)
    XYFile.write(format(x0, 'b').zfill(NBit) + " " + format(x1, 'b').zfill(NBit) + " " + format(x2, 'b').zfill(NBit) + " " + format(x3, 'b').zfill(NBit) + " " + format(x4, 'b').zfill(NBit) + " " + format(x5, 'b').zfill(NBit) + " " + format(y0, 'b').zfill(NBit) + " " + format(y1, 'b').zfill(NBit) +  " " + format(y2, 'b').zfill(NBit) +  " " + format(y3, 'b').zfill(NBit)+  " " + format(y4, 'b').zfill(NBit) +  " " + format(y5, 'b').zfill(NBit)+"\n")
...
plt.ylabel(r'\textit{x0(t), x1(t), x2(t), x3(t), x4(t), x5(t)}', fontsize=20)
leg = plt.legend(bbox_to_anchor=(1.05, 1.0), loc='upper left')
plt.tight_layout()
plt.grid(True)
plt.savefig('X6Spin_software.eps', format='eps')
plt.show()
...
plt.title(r'\textbf{Adiabatic Bifurcation}', fontsize=20)
plt.xlabel(r'\textit{t}', fontsize=20)
plt.ylabel(r'\textit{y0(t), y1(t), y2(t), y3(t), y4(t), y5(t)}', fontsize=20)
leg = plt.legend(bbox_to_anchor=(1.05, 1.0), loc='upper left')
plt.tight_layout()
plt.grid(True)
plt.savefig('Y6Spin_software.eps', format='eps')
plt.show()
