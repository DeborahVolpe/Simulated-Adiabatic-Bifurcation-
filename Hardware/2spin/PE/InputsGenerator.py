...
for i in range(numberSteps):
	...
	SumVect = ((s3Aa * JMatrix_xi) >> shiftVect)
	sum = int(SumVect.item(1))
	if sum < 0:
		sum = 2**NBit + sum
	SumFile.write(format(sum, 'b').zfill(NBit) + "\n")
	StateFile.write("In iteration " + format(i) + " in the state3 Aa: " + str(s3Aa) + "\n")
	s4Ma = (K * s3Aa) >> shiftVect
	s4Mb = np.multiply(s1Aa,s3Aa) >> shiftVect
	StateFile.write("In iteration " + format(i) + " in the state4 Ma: " + str(s4Ma) + " Mb: " + str(s4Mb) + "\n")
	...
	s8Aa = s7Aa + SumVect
	StateFile.write("In iteration " + format(i) + " in the state8 Aa: " + str(s8Aa) + "\n")
	s9Ma = (s8Aa * deltaT) >> shiftVect
	StateFile.write("In iteration " + format(i) + " in the state9 Ma: " + str(s9Ma) + "\n")
	s10Aa = yVector-s9Ma
	StateFile.write("In iteration " + format(i) + " in the state10 Aa: " + str(s10Aa) + "\n")
